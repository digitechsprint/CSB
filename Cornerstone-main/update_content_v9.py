...
(function() {
    ...
    const translations = {
        ...
        // General Card title translations (fallback)
        "Hillside View House": "Graphix Tower – Grade-A IT/ITES",
        "Modern Hillside Home": "Signia Tower (Majestic Signia)",
        "Dallas Townhouse": "Estate 361 (Max Estate)",
        "Urban Modern House": "The Arista Lux (Nimbus)",
    ...
    function populateCard(card, prop) {
        ...
        if (areaLabel) areaLabel.textContent = ""; // Hide the trailing "sq ft" label
    }
    ...
    function renderPropertyGrid() {
        ...
        if (!cardTemplate) {
            ...
            if (firstCard) {
                // Ensure we clone a card with original Homy text structure
                const titleEl = firstCard.querySelector('.framer-1xf9if4 p');
                if (titleEl && (titleEl.textContent.includes("Hillside") || titleEl.textContent.includes("Residence") || titleEl.textContent.includes("Studio") || titleEl.textContent.includes("Villa") || titleEl.textContent.includes("DLF") || titleEl.textContent.includes("Max") || titleEl.textContent.includes("Nimbus"))) {
        ...
        // Mark grid container as rendered
        const gridNode = document.querySelector('.framer-1hh8qsi');
        if (gridNode) {
            gridNode.dataset.customRendered = "true";
        }
    }

    function checkAndCustomizeSidebar() {
        // 1. Hide Price Range and Bedrooms filter sections
        const priceSec = document.querySelector('.framer-jrohvw');
        if (priceSec && priceSec.style.display !== 'none') priceSec.style.display = 'none';

        const bedSec = document.querySelector('.framer-kxq65d');
        if (bedSec && bedSec.style.display !== 'none') bedSec.style.display = 'none';

        // 2. Customize Property Type buttons
        ...
        if (needsCatSetup) {
            catContainers.forEach((container, idx) => {
                if (idx < catLabels.length) {
                    ...
                    container.style.display = 'none'; // Hide townhouse/extra buttons
                }
            });
        }

        // 3. Customize Location buttons
        ...
        let needsLocSetup = false;
        locContainers.forEach((container, idx) => {
            if (idx < locLabels.length) {
                const textEl = container.querySelector('p');
                if (textEl && textEl.textContent !== locLabels[idx]) {
                    needsLocSetup = true;
                }
            }
        });
        ...
        if (needsLocSetup) {
            locContainers.forEach((container, idx) => {
                if (idx < locLabels.length) {
                    ...
                    container.style.display = 'none'; // Hide Chicago, Seattle, San Francisco
                }
            });
        }

        // 4. Customize Clear All button
    ...
    function populateHomepageCards() {
        const cards = document.querySelectorAll('.framer-271b1e-container .framer-tvrfza') || document.querySelectorAll('[data-framer-name="Property card"]');
        if (cards.length > 0) {
            cards.forEach((card, idx) => {
                if (idx < customProperties.length) {
                    populateCard(card, customProperties[idx]);
                }
            });
        }
    }

    const runGlobalPageScripts = () => {
        const path = window.location.pathname.toLowerCase();
        if (path.includes('properties')) {
            checkAndCustomizeSidebar();
            
            // Re-render grid if original Homy properties appear (indicating React re-hydration)
            const testCard = document.querySelector('.framer-tvrfza');
            const titleEl = testCard ? testCard.querySelector('.framer-1xf9if4 p') : null;
            const isOriginal = titleEl && (
                titleEl.textContent === "Hillside View House" || 
                titleEl.textContent === "Forest Side Residence" || 
                titleEl.textContent === "Modern Hillside Home" || 
                titleEl.textContent === "Dallas Townhouse" || 
                titleEl.textContent === "Urban Modern House"
            );
            
            const gridNode = document.querySelector('.framer-1hh8qsi');
            const isNotRendered = gridNode && gridNode.dataset.customRendered !== "true";

            if (isOriginal || isNotRendered) {
                renderPropertyGrid();
            }
