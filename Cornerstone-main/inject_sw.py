...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', function() {
        navigator.serviceWorker.register('/sw.js').then(function(registration) {
            ...
            if (!sessionStorage.getItem('sw_installed')) {
                sessionStorage.setItem('sw_installed', '1');
                setTimeout(() => window.location.reload(), 500); // Reload once to let SW intercept
            }
        }, function(err) {
...
def clean_old_script(content):
    content = re.sub(r'<script>\s*\(function\(\) \{\s*const translations = \{.*?<\/script>', '', content, flags=re.DOTALL)
    content = re.sub(r'<script>\s*if \(\'serviceWorker\' in navigator\) \{.*?<\/script>', '', content, flags=re.DOTALL)
    return content
...
def process_file(filepath):
    ...
    content = clean_old_script(content)
    
    # Static replacements (only replaces if the whole string is found together)
    for old, new in translations.items():
        content = content.replace(old, new)
        
    # Inject Service Worker Registration
    content = content.replace('</head>', SW_REGISTRATION + '\n</head>')
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Injected SW into: {filepath}")
